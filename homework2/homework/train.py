    ...
    **kwargs,
    ...
    for epoch in range(num_epoch):
        ...
        for img, label in train_data:
            ...
            train_accuracy = (preds==label).float().mean()
            metrics['train_acc'].append(train_accuracy.item())
            logger.add_scalar('img_train_acc', train_accuracy, global_step)
        ...
        with torch.inference_mode():
            ...
            for img, label in val_data:
                ...
                #forward pass
                outputs_val = model(img)
                _,preds_val = torch.max(outputs_val,1)
                #get validation accuracy
                val_accuracy = (preds_val==label).float().mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=1e-3*1.05)
